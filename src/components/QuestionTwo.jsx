...
function DraggableObject({ name, onDrop, children, ...props }) {
  ...
  useFrame(() => {
    if (dragging && raycaster.ray.intersectPlane(plane, intersection)) {
  ...
  const onPointerUp = () => {
    ...
    onDrop(name, ref.current.position, fadeOut);
  };
  ...
  return (
      <group ref={ref} {...props} onPointerDown={onPointerDown} onPointerUp={onPointerUp}>
        <group scale={[1, 1, 1]} visible={opacity > 0}>
          {React.cloneElement(children, { opacity })}
        </group>
      </group>
  );
}
...
const QuestionTwo = () => {
  ...
  const handleDrop = (name, position, fadeOut) => {
    ...
    if (distanceToOcean > oceanRadius) {
      ...
      const industries = {
        ...
        battery: "baterias usadas",
        bottle: "botellas de plástico",
      };
  ...
  return (
      ...
      >
        <div className="absolute inset-0 backdrop-blur-md bg-black bg-opacity-30 pointer-events-none"></div>

        <Canvas className="relative pointer-events-auto" style={{ width: "100vw", height: "100vh" }}>
          <OrthographicCamera makeDefault position={[0, 0, 10]} near={0.1} far={100} zoom={130} />
          ...
          <Text position={[-6.2, 1.5, 0]} fontSize={0.25} color="white" anchorX="left" anchorY="top">
        ...
        {showModal && (
            ...
            >
              <div className="text-center text-white max-w-lg mx-auto">
                {message === "battery" ? (
                    <>
                      <h1 className="text-4xl font-bold">Acertaste, llevas 2/3 respuestas correctas</h1>
                ...
                <p className="mt-6 text-lg">
                  ...
                  <br />• Una cáscara de fruta: <strong>contaminante menor</strong>. Biodegradable, pero puede afectar el ecosistema.
                  <br />• Una batería usada: <strong>Contaminante severo</strong>. Libera metales pesados y sustancias tóxicas.
                </p>
                <button
